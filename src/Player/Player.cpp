...
void Player::Update(const float deltaTime){
    ...
    const glm::vec3 newPosition = position + playerVelocity * deltaTime;
...
void Player::calculateNewPositionY(const float& deltaTime) {
...
void Player::applyNewPositionY(const glm::vec3 &newPosition) {

    const auto newYposGround = glm::vec3(position.x, newPosition.y, position.z);
    const bool isNewPosGrounded = checkNewPositionY(newYposGround);
    const auto newYPos = glm::vec3(position.x, newPosition.y + HEIGHT, position.z);
...
void Player::UpdatePositionXZ(const glm::vec3& newPosition) {
    ...
    {
        if((isShifting && checkNewPositionY(newPosition)) || (isShifting && !isGrounded) || (!isShifting))
        {
            position.x = newPosition.x;
            position.z = newPosition.z;
        }
        else if(checkNewPositionY(newPosX))
        {
            position.x = newPosition.x;
        }
        else if(checkNewPositionY(newPosZ)){
            position.z = newPosition.z;
        }
        ...
        {
            if((isShifting && checkNewPositionY(newPosX)) || (isShifting && !isGrounded) || !isShifting)
            {
                position.x = newPosition.x;
            }
            else
        ...
        {
            if((isShifting && checkNewPositionY(newPosZ)) || (isShifting && !isGrounded) || !isShifting)
            {
                position.z = newPosition.z;
            }
            else
...
void Player::UpdateDeceleration(const float &deltaTime){
    ...
    if (velocityMagnitudeXZ > 0) {
        ...
        const float deceleration = 25 * deltaTime;
        ...
        const glm::vec2 velocityXZ = glm::normalize(glm::vec2(playerVelocity.x, playerVelocity.z)) * velocityMagnitudeXZ;
...
// Ensure the total velocity doesn't exceed the maximum speed
    if (glm::length(glm::vec2(playerVelocityXZ)) > movementSpeed) {
        const glm::vec2 normalizedXZ = glm::normalize(glm::vec2(playerVelocity.x, playerVelocity.z));
    ...
    if (dir == cameraMovement::UP) {
        if((isGrounded && !isJumping) || isSwimming)
        {
            playerVelocity.y = 0;
            isJumping = true;
            if(isSwimming && !isColliding(position + playerVelocity * deltaTime))
...
bool Player::isColliding(const glm::vec3 newPosition) const //front can be x or z since they are checked separately
{
    //by default false
...
bool Player::checkNewPositionZ(const float newZ) const
{
    for(int i = -1; i <= 1; i+= 2) {
        const float xWidth = static_cast<float>(i) * WIDTH;

        const float zWidth = (newZ > position.z) ? WIDTH + 0.01f : -WIDTH - 0.01f;
        ...
        if(currentChunk != nullptr) {
            ...
            const int newChunkPosX = static_cast<int>(glm::round(
                    (position.x - static_cast<float>(currentChunk->chunkPosition.x * Chunk::SIZE)) + xWidth));

            const int y1 = static_cast<int>(glm::round(position.y));
            const int y2 = static_cast<int>(glm::round(position.y - 1));
            const int y3 = static_cast<int>(glm::round(position.y - 1.5));
...
bool Player::checkNewPositionX(const float newX) const
{
    for(int i = -1; i <= 1; i+= 2) {
        const float zWidth = static_cast<float>(i) * WIDTH;

        const float xWidth = (newX > position.x) ? WIDTH + 0.01f : -WIDTH - 0.01f;
        glm::ivec2 _chunkPosition;
...
bool Player::checkNewPositionXZ(const glm::vec3& newPosition) const
{

    glm::ivec2 _chunkPosition;

    const float _widthZ = (newPosition.z > position.z) ? WIDTH + 0.01f : -WIDTH - 0.01f;
    const float _widthX = (newPosition.x > position.x) ? WIDTH + 0.01f : -WIDTH - 0.01f;
    ...
    if(currentChunk != nullptr) {
        ...
        newChunkPos.x = ((newPosition.x - static_cast<float>(currentChunk->chunkPosition.x * Chunk::SIZE)) + _widthX);
...
bool Player::checkNewPositionY(const glm::vec3& newPosition) const
{
    for(int i = -1; i <= 1; i+= 2)
    {
        for(int j = -1; j <= 1; j+= 2)
        {
            ...
            const float xWidth = static_cast<float>(i) * WIDTH;
            const float zWidth = static_cast<float>(j) * WIDTH;
...
void Player::setBlockID(const int blockID) {
    currentBlockID = blockID;
}

int Player::getBlockID() const {
    return currentBlockID;
}

glm::vec3 Player::positionInChunk() const {
    glm::vec3 localChunkPos; //position in chunk
    localChunkPos.x = ((position.x - static_cast<float>(chunkPosition.x * Chunk::SIZE)));
    localChunkPos.z = ((position.z - static_cast<float>(chunkPosition.y * Chunk::SIZE)));
    localChunkPos.y = position.y;
    return localChunkPos;
}
bool Player::checkCollisionWithBlockLocal(const glm::ivec3 localPos) const{
    const float _widthZ = (static_cast<float>(localPos.z) > positionInChunk().z) ? WIDTH + 0.01f : -WIDTH - 0.01f;
    const float _widthX = (static_cast<float>(localPos.x) > positionInChunk().x) ? WIDTH + 0.01f : -WIDTH - 0.01f;

    if(static_cast<float>(localPos.x) != glm::round(positionInChunk().x + _widthX) ||
            static_cast<float>(localPos.z) != glm::round(positionInChunk().z + _widthZ) ||
            (static_cast<float>(localPos.y) != glm::round(positionInChunk().y - 1.5f) && static_cast<float>(localPos.y) != glm::round(positionInChunk().y))){
        return false;
    }
    else{
        return true;
...
void Player::savePosToFile() const {
    // Serialize position and camera.front
    constexpr size_t dataSize = sizeof(position) + sizeof(camera.Front) + sizeof(toolbar->toolbarItems);
